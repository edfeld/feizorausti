...
try {
	...
	//retrieve comments that are flagged
	$query_getFlaggedComments = $conCreative->prepare("SELECT * FROM comments WHERE isFlagged=1 AND isDeleted=0");
	$query_getFlaggedComments->execute();
	$totalRows_getFlaggedComments = $query_getFlaggedComments->rowCount();
}
...
<body>
	<div class="row-fluid bodycontent">
		...
		<div class="span9 maincontainer shadow">
			...
			Comments that have been flagged as innapropriate by visitors and need to be reviewed are listed below. If you opt to un-flag them, they will be removed from this list. If you decide to delete them, they will be archived but hidden from the public.
			<hr />
			<?php
				$counterComments=0;
				if($totalRows_getFlaggedComments > 0){
					while($row = $query_getFlaggedComments->fetch(PDO::FETCH_ASSOC)){
						//parse the date
						...
						<div class='well'>
							...
							<div class="commentButtons btn-toolbar">
								<div class="btn-group">
									<button class='btn btn-mini' onclick='approveComment(<?php echo $row['commentID']; ?>);'>Un-Flag</button>
				...
				if($counterComments <= 0){
					echo "<div class='alert'>No flagged comments were found.</div>";
				}
			 ?>
		</div>
	</div>
